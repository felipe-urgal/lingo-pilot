...
test("prod check recusa runtime e testes no mesmo banco", () => {
  const same = "postgresql://user:pass@127.0.0.1:5435/same_test";
  assert.throws(
    () =>
      createCheckEnvironment({
        CHECK_DATABASE_URL: same,
        CHECK_TEST_DATABASE_URL: same,
      }),
...
test("produção exige conexão PostgreSQL explícita e readiness HTTPS", () => {
  ...
  assert.throws(
    () => productionReadyUrl({ LINGO_PRODUCTION_READY_URL: "http://example.com/ready" }),
    /https:\/\//,
  );
...
test("scripts de produção não carregam .env.local implicitamente", async () => {
  ...
  for (const name of ["prod:check", "prod:migrate", "prod:verify", "prod:backup"]) {
...
test("readiness valida PostgreSQL e schema mínimo sem expor diagnóstico", async () => {
  const route = await readFile("apps/web/app/api/health/ready/route.ts", "utf8");
