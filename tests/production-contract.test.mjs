...
const manifestUrl = new URL('../.dev-dashboard/production.json', import.meta.url);
const gateUrl = new URL('../scripts/production-gate.mjs', import.meta.url);

const manifest = JSON.parse(await readFile(manifestUrl, 'utf8'));

test('Production Contract permanece fail-closed até os blockers serem resolvidos', () => {
...
test('prod:status informa blockers sem falhar', () => {
  ...
  assert.match(result.stdout, /Produção do LingoPilot ainda está bloqueada por contrato/);
...
test('prod:check falha de propósito enquanto produção não está pronta', () => {
  ...
  assert.match(result.stderr, /Produção do LingoPilot não está pronta para habilitação/);
});
